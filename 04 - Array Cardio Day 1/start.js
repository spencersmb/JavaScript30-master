...
const names = inventors.map( item => {
  return {firstName: item.first, lastName: item.last};
});
...
// const linked = category.querySelector('a');

const link = document.querySelector('.link');
console.log(link.innerHTML);

const de = streetNames.filter( street => street.includes('de'));
console.log(de);


// 7. sort Exercise
// Sort the people alphabetically by last name
const lastNames = people.sort( (lastOne, nextOne) => {

  //First full name - destructure it by splitting the 2 names into seperate arrays
  //this allows us to desructure the name without needing to save it as a seperate array
  const [aLast, aFirst] = lastOne.split(', ');
  const [bLast, bFirst] = nextOne.split(', ');

  //then compare then and return 1 or -1
  //1 is for greater so smallest to largest
...
//Reduce loops through an array and collects items from the array into an object
const transportation = data.reduce( (obj, item) => {
  //check if object key exists then add it
  if(!obj[item]){
    obj[item] = 0; //starting point
  }

  //increase item count for each item in the array
  obj[item]++;

  return obj;

}, {} ); //empty object at the end that we'll store the count in.
